...
export default defineComponent({
  ...
  setup() {
    ...
    const count = computed(() =>
      store.state.gameSetting?.roles?.some((role: Role) => role.color === "grey") ? 2 : 1
    );
    ...
    function groupedRoles(roles: GroupedRoles[]) {
      const roleCount = store.state.gameSetting?.roles?.reduce(
        (acc: number, item: GroupedRoles) => acc + item.roles.length,
        0
      );
