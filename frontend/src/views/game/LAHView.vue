...
export default defineComponent({
  ...
  setup() {
    ...
    function groupedRoles(roles: GroupedRoles[]) {
      const filterRolesByColor = (color: string) =>
        _.flatMap(roles, (item) => _.filter(item.roles, { color }));
      const blueRoles = filterRolesByColor("blue");
      const redRoles = filterRolesByColor("red");
      const greyRoles = _.flatMap(roles, (item) => _.filter(item.roles, { color: "grey" })).sort(
        (a, b) => a.pair - b.pair
      );
      const [specialGreyRoles, normalGreyRoles] = _.partition(greyRoles, (role) =>
        [18, 19].includes(role.pair)
      );
      const firstQuadrantData = [...redRoles, ...blueRoles];
      const [secondQuadrantData, thirdQuadrantData] = _.partition(normalGreyRoles, (role) =>
        [12, 13, 14].includes(role.pair)
      );
