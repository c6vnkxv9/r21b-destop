...
export default function CharacterPairCard(props: CharacterPairCardProps): JSX.Element {
  const { pair, isSelected, onSelect, onEdit, onDelete, showTitle = true, showDesc = true, showPic = true } = props;
  ...
  const getSideColors = (label: Role["side"]): { bg: string; fg: string } => {
    ...
    if (label === "blue") return { bg: theme.palette.secondary.dark, fg: theme.palette.secondary.light };
  ...
  return (
    <Box onClick={onSelect} sx={{ borderRadius: 2, p: isSelected ? 0.75 : 0, transition: "all .25s ease" ,
      background: isSelected
        ? `linear-gradient(135deg, ${theme.palette.primary.main} 0%, ${theme.palette.secondary.main} 100%)`
        : "transparent",
      border: isSelected ? "none" : `2px solid ${alpha(theme.palette.common.white, 0.12)}`,
      "&:hover": !isSelected ? { borderColor: alpha(theme.palette.common.white, 0.22), background: `linear-gradient(135deg, ${alpha(theme.palette.primary.main, 0.15)} 0%, ${alpha(theme.palette.secondary.main, 0.15)} 100%)` } : undefined,
    }}>
      <Box
        sx={{
          position: "relative",
      ...
      {showPic && (
        <Box sx={{ display: "flex", justifyContent: "center", mb: 1 }}>
          <AvatarGroup sx={{ "& .MuiAvatar-root": { borderColor: alpha(theme.palette.common.white, 0.18) } }}>
